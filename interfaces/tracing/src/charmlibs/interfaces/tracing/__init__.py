...
``TracingEndpointRequirer`` object only requires instantiating it, typically in the
constructor of your charm. The ``TracingEndpointRequirer`` constructor requires the name of
the relation over which a tracing endpoint is exposed by the Tempo charm, and a list of
protocols it intends to send traces with. This relation must use the ``tracing`` interface.
...
Note that the first argument (``self``) to ``TracingEndpointRequirer`` is always a reference
to the parent charm.
...
If the ``protocol`` is not in the list of protocols that the charm requested at endpoint
set-up time, the library will raise an error.
