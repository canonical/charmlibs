...
By default, RollingOps uses: ``/var/lib/rollingops``. Meaning that all the
artifacts will be found under ``<charm-dir>/var/lib/rollingops``.
...
For rootless charms, the default path (``/var/lib/rollingops``) may not be writable by the
charm process. In these cases, either configure ``base_dir`` to a writable location, or ensure
the default path exists and is writable within the charm's runtime environment (for example,
by including it in the snap or rock).
